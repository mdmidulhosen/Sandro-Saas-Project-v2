...
function upgradeLegacyCategory(category = {}) {
  return newCategory({
    id: category.id || uid(),
    ...
    copiesPerRank: Number.isFinite(category.winnersPerPosition)
      ...
      : Number.isFinite(category.copiesPerRank)
        ...
        : 1,
    podiumPreset: category.podiumPreset || "class",
    ...
    trophyAlignment: category.trophyAlignment || "right",
...
export function normalizeTemplate(config = {}) {
  const categories = Array.isArray(config.categories)
    ? config.categories.map(upgradeLegacyCategory)
    : [newCategory()];
  ...
  return {
    ...
    podiumPreset: config.podiumPreset || config.podioMode || "class",
...
function getBreakValue(category, field) {
  if (!field) return "";
  if (field === "reference") return normalizeText(category.reference);
      ...
      for (let rank = 1; rank <= Math.max(0, category.medals || 0); rank += 1) {
        for (let copy = 0; copy < copiesPerRank; copy += 1) {
          labels.push({
            ...
            tag: `${category.reference} - medal ${rank}`,
          });
        }
      }
    }
...
export function buildSheetSvgString(
  ...
  positions.forEach(({ x, y, width, height, svg, id }) => {
    const viewBox = svg.getAttribute("viewBox") || `0 0 ${width} ${height}`;
    output += `  <g id="sticker-${id}">\n`;
    output += `    <svg x="${x}" y="${y}" width="${width}" height="${height}" viewBox="${viewBox}">\n`;
