...
export default function GeneratorPage() {
  ...
  const [eventName, setEventName] = useState(
    "1° PROVA CAMPIONATO REGIONALE FGI GAF",
  );
  ...
  const [customPodioTexts, setCustomPodioTexts] = useState({
    1: "",
    2: "",
    3: "",
  });
  ...
  function addCategory() {
    setCategories((prev) => [
      ...prev,
      newCategory({ name: "Nuova Categoria", apparatus: [] }),
    ]);
  }
  ...
  function updateCat(idx, field, value) {
    setCategories((prev) => {
      ...
      if (
        field === "hasApparatus" &&
        value &&
        (!cats[idx].apparatus || cats[idx].apparatus.length === 0)
      ) {
        cats[idx].apparatus = [];
      }
      return cats;
  ...
  function handleDownloadSvg() {
    if (previewRef.current) {
      downloadSvgSheet(
        previewRef.current,
        `etichette-${activeGroup || "tutte"}.svg`,
        getSheetW(),
        labelSpacingMm,
      );
    }
    ...
    const newCats = excelRows
      .map((row) => {
        ...
        return newCategory({
          ...
          diameterMm: colMap.diameter
            ? parseInt(getVal(row, colMap.diameter)) || 40
            : 40,
          winnersPerPosition: colMap.winners
            ? parseInt(getVal(row, colMap.winners)) || 1
            : 1,
          apparatus: [],
        });
      })
      ...
      <div className="label-wrapper" key={`${label.type}-${idx}`}>
        {label.type === "medal-front" && (
          <MedalLabel
            ...
            bottomText={label.bottomText}
            logoSrc={logoSrc}
            diameterMm={label.diameterMm}
            textPosition={label.textPosition || "inside"}
          />
        )}
  ...
  const labelCountText = (n) =>
    `${n} ${n === 1 ? t("gen_label_single") : t("gen_label_plural")}`;
    ...
    <div className="page">
        ...
        <div className="form-grid">
          <div className="field">
            ...
            <input
              ...
              placeholder="1° PROVA CAMPIONATO..."
            />
          </div>
          <div className="field">
            ...
            <input
              ...
              placeholder="Città, GG Mese AAAA"
            />
          </div>
          <div className="field">
          ...
          <div className="field">
            ...
            <select
              value={podioMode}
              onChange={(e) => setPodioMode(e.target.value)}
            >
              {PODIO_MODES.map((m) => (
          ...
          {[1, 2, 3].map((pos) => (
            <div className="field" key={pos}>
              <label>
                {pos === 1
                  ? t("tmpl_custom_podio_1")
                  : pos === 2
                    ? t("tmpl_custom_podio_2")
                    : t("tmpl_custom_podio_3")}
              </label>
              <input
                value={customPodioTexts[pos] || ""}
                placeholder={t("tmpl_custom_podio_ph")}
                onChange={(e) =>
                  setCustomPodioTexts((prev) => ({
                    ...prev,
                    [pos]: e.target.value,
                  }))
                }
          ...
          {hasParticipation && (
            <>
              <div className="field field-checkbox">
                ...
                <input
                  type="checkbox"
                  checked={participationDateBottom}
                  onChange={(e) => setParticipationDateBottom(e.target.checked)}
                />
              </div>
              <div className="field">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setParticipationCount(parseInt(e.target.value) || 1)
                  }
                  style={{ width: 100 }}
                />
              </div>
            ...
            <div key={cat.id} className="cat-table-row-group">
              <div className="cat-table-row">
                ...
                <input
                  ...
                  value={cat.coppe}
                  onChange={(e) =>
                    updateCat(i, "coppe", parseInt(e.target.value) || 0)
                  }
                />
                <input
                  type="number"
                  ...
                  value={cat.medals}
                  onChange={(e) =>
                    updateCat(i, "medals", parseInt(e.target.value) || 0)
                  }
                />
                <select
                  className="cat-input-sm"
                  value={cat.diameterMm}
                  onChange={(e) =>
                    updateCat(i, "diameterMm", parseInt(e.target.value))
                  }
                >
                ...
                <input
                  ...
                  max="20"
                  className="cat-input-sm"
                  value={cat.winnersPerPosition || 1}
                  onChange={(e) =>
                    updateCat(
                      i,
                      "winnersPerPosition",
                ...
                <input
                  type="checkbox"
                  checked={cat.hasApparatus}
                  onChange={(e) =>
                    updateCat(i, "hasApparatus", e.target.checked)
                  }
                  title={t("gen_col_apparatus")}
                />
                <button
              ...
              {cat.hasApparatus && (
                <div className="apparatus-section">
                  ...
                  <div className="apparatus-presets">
                    <span
                      style={{ fontSize: 11, color: "#888", marginRight: 6 }}
                    >
                      Preset:
                    </span>
                    <button
                      ...
                      onClick={() => addApparatusPreset(i, "gaf")}
                    >
                      {t("tmpl_apparatus_preset_gaf")}
                    </button>
                    <button
                      type="button"
                      className="btn btn-xs btn-outline"
                      onClick={() => addApparatusPreset(i, "ritmica")}
                    >
                      {t("tmpl_apparatus_preset_rg")}
                    </button>
                    <button
                      type="button"
                      className="btn btn-xs btn-outline"
                      onClick={() => addApparatusPreset(i, "trampo")}
                    >
                      {t("tmpl_apparatus_preset_trampo")}
                    </button>
                  </div>
                    ...
                    {(cat.apparatus || []).map((app, ai) => (
                      <div key={ai} className="app-chip">
                        ...
                        <button
                          ...
                          onClick={() => removeApparatus(i, ai)}
                        >
                          {t("tmpl_apparatus_remove")}
                        </button>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="apparatus-add-row">
                    ...
                    <button
                      className="btn btn-sm btn-outline"
                      onClick={() => addApparatus(i)}
                    >
                      {t("tmpl_apparatus_add")}
                    </button>
                  </div>
                </div>
              )}
      ...
      <div className="gen-buttons">
        <button
          ...
          onClick={() => generate("partecipazione")}
        >
          {t("gen_btn_participation")}
        </button>
        <button className="btn btn-gen" onClick={() => generate("coppe")}>
          {t("gen_btn_trophies")}
        </button>
        <button className="btn btn-gen" onClick={() => generate("medaglie")}>
          {t("gen_btn_medals")}
        </button>
        <button
          className="btn btn-gen btn-gen-all"
      ...
      {labels.length > 0 && (
        <div id="preview-area" className="preview-section">
          <div className="preview-header">
            <h2>
              {t("gen_preview_title")} – {labelCountText(labels.length)}
            </h2>
            <div className="preview-actions">
              <div className="sheet-size-row">
                ...
                {sheetSizeKey === "custom" && (
                  <>
                    <input
                      ...
                      value={customW}
                      onChange={(e) =>
                        setCustomW(parseInt(e.target.value) || 297)
                      }
                ...
                <span className="sheet-size-label" style={{ marginLeft: 12 }}>
                  {t("gen_spacing_label")}:
                </span>
                <input
                  type="number"
                  min="0"
                  max="30"
                  value={labelSpacingMm}
                  onChange={(e) =>
                    setLabelSpacingMm(parseInt(e.target.value) || 0)
                  }
                  className="sheet-size-input"
                  style={{ width: 56 }}
                />
          ...
          <div className="modal modal-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <p className="mb-16">
                {t("excel_loaded")}{" "}
                <strong>
                  {excelRows.length} {t("excel_rows")}
                </strong>
                ,{" "}
                <strong>
                  {excelHeaders.length} {t("excel_cols")}
                </strong>
                . {t("excel_map_desc")}
              </p>
              <div className="form-grid">
                ...
                <div className="excel-preview">
                  ...
                  {excelRows.length > 5 && (
                    <p className="table-more">
                      … {excelRows.length - 5} {t("excel_rows")}
                    </p>
                  )}
                </div>
              )}
