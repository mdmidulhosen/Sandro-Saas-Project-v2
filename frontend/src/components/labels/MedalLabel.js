...
export default function MedalLabel({
  ...
  if (textPosition === 'outside') {
    const VB_W = 200;
    const VB_H = 320;
    const cx = 100, cy = 165;  // circle center shifted down to leave room for top text
    const r = 93;
    const cr = 80;
    const lSz = cr * 2;
    ...
    return (
      ...
      >
        ...
        {topText && (
          <text x={cx} y={yLine1} fontSize={fsTop} fontFamily={FONT} fontWeight="bold"
            textAnchor="middle" fill="black">
            {line1}
          </text>
        )}
        {hasLine2 && (
          <text x={cx} y={yLine2} fontSize={fsTop} fontFamily={FONT} fontWeight="bold"
            textAnchor="middle" fill="black">
            {line2}
          </text>
        )}
        ...
        {/* Logo inside circle */}
        {logoSrc && (
          <image href={logoSrc} x={cx - cr} y={cy - cr} width={lSz} height={lSz}
            preserveAspectRatio="xMidYMid meet" clipPath={`url(#${uid}-c)`} />
        )}

        {/* Bottom text – date / location */}
        {bottomText && (
          <text x={cx} y={yBot} fontSize={fsBot} fontFamily={FONT} fontWeight="normal"
            textAnchor="middle" fill="black">
  ...
  return (
    ...
    >
      ...
      {/* Top curved text */}
      {topText && (
        <text fontSize={fs.top} fill="black" fontFamily={FONT} fontWeight="bold" textAnchor="middle">
          <textPath href={`#${uid}-t`} startOffset="50%">{topText.toUpperCase()}</textPath>
        </text>
      )}

      {/* Bottom curved text */}
      {bottomText && (
        <text fontSize={fs.bot} fill="black" fontFamily={FONT} fontWeight="bold" textAnchor="middle">
          <textPath href={`#${uid}-b`} startOffset="50%">{bottomText.toUpperCase()}</textPath>
        </text>
      )}
    </svg>
  );
